...
export const VIDEOS = {
  ...
  16: { title:"Match/Case in Python", url:"https://www.youtube.com/watch?v=scNNi4860kk" },
};
...
export const chapters = [
  ...
  { id:4, emoji:"❓", title:"If / Else Decisions", color:"#B44FF7", level:"Beginner",
    ...
    quiz:[
      ...
      { q:"Which is the CORRECT way to write 'else if' in Python?", options:["else if","elseif","elsif","elif"], ans:3 },
    ]
  },
  ...
  { id:16, emoji:"🔀", title:"Match / Case (Switch)", color:"#E91E63", level:"Advanced",
    content:`In other languages (JavaScript, Java, C++), you have a \`switch\` statement for checking many values. Python didn't have this for years — you had to use long \`if/elif\` chains. But **Python 3.10** added \`match/case\`!\n\n**The problem — too many elif's gets ugly:**\n\`\`\`python\n# This works, but it's messy with many options:\nday = "Monday"\nif day == "Monday":\n    print("Start of the week")\nelif day == "Tuesday":\n    print("Second day")\nelif day == "Wednesday":\n    print("Midweek")\nelif day == "Thursday":\n    print("Almost Friday")\nelif day == "Friday":\n    print("TGIF!")\nelse:\n    print("Weekend!")\n\`\`\`\n\n**The solution — match/case:**\n\`\`\`python\nday = "Monday"\nmatch day:\n    case "Monday":\n        print("Start of the week")\n    case "Tuesday":\n        print("Second day")\n    case "Wednesday":\n        print("Midweek")\n    case "Friday":\n        print("TGIF!")\n    case _:\n        print("Some other day")\n\`\`\`\n\nThe \`_\` (underscore) is the **wildcard** — it matches everything else, just like \`else\` in an if/elif chain. Always put it last!\n\n**Matching multiple values in one case:**\n\`\`\`python\nday = "Saturday"\nmatch day:\n    case "Monday" | "Tuesday" | "Wednesday" | "Thursday" | "Friday":\n        print("Weekday — time to work! 💼")\n    case "Saturday" | "Sunday":\n        print("Weekend — relax! 🎉")\n\`\`\`\n\nThe \`|\` (pipe) means OR — so \`"Saturday" | "Sunday"\` means "if it's Saturday OR Sunday".\n\n**Matching with variables (capturing):**\n\`\`\`python\ncommand = ("move", 5, 10)\nmatch command:\n    case ("move", x, y):\n        print(f"Moving to ({x}, {y})")\n    case ("shoot", direction):\n        print(f"Shooting {direction}")\n    case ("heal",):\n        print("Healing!")\n    case _:\n        print("Unknown command")\n\`\`\`\n\nHere \`x\` and \`y\` **capture** the values from the tuple. So \`x = 5\` and \`y = 10\`.\n\n**Matching with conditions (guards):**\n\`\`\`python\nscore = 85\nmatch score:\n    case x if x >= 90:\n        print("Grade: A 🌟")\n    case x if x >= 80:\n        print("Grade: B 👍")\n    case x if x >= 70:\n        print("Grade: C 😊")\n    case _:\n        print("Grade: F 😢")\n\`\`\`\n\nThe \`if\` after the case is called a **guard** — it adds an extra condition.\n\n**Matching types:**\n\`\`\`python\nvalue = 42\nmatch value:\n    case int():\n        print(f"{value} is an integer")\n    case str():\n        print(f"{value} is a string")\n    case list():\n        print(f"{value} is a list")\n    case _:\n        print("Something else")\n\`\`\`\n\n**When to use match/case vs if/elif:**\n- Use \`match/case\` when checking ONE value against MANY possible exact matches\n- Use \`if/elif\` when you have complex conditions with \`and\`, \`or\`, comparisons\n- \`match/case\` is cleaner for menus, commands, HTTP status codes, game actions\n\n⚠️ **Important:** \`match/case\` requires **Python 3.10 or newer**! If you get a SyntaxError, check your Python version with \`python --version\`.`,
    tryIt:[
      { after:1, prompt:"Try match/case with different day values!", starter:'day = "Friday"\nmatch day:\n    case "Monday":\n        print("Start of week")\n    case "Friday":\n        print("TGIF!")\n    case "Saturday" | "Sunday":\n        print("Weekend!")\n    case _:\n        print("Regular day")' },
      { after:3, prompt:"Try matching a tuple command!", starter:'command = ("move", 3, 7)\nmatch command:\n    case ("move", x, y):\n        print(f"Moving to ({x}, {y})")\n    case ("heal",):\n        print("Healing!")\n    case _:\n        print("Unknown")' },
      { after:5, prompt:"Build a grade checker with match/case guards!", starter:'score = 92\nmatch score:\n    case x if x >= 90:\n        print(f"{x} = Grade A")\n    case x if x >= 80:\n        print(f"{x} = Grade B")\n    case x if x >= 70:\n        print(f"{x} = Grade C")\n    case _:\n        print(f"{score} = Grade F")' },
    ],
    quiz:[
      { q:"What does _ (underscore) mean in match/case?", options:["It's an error","It matches everything (default)","It means 'nothing'","It stops the match"], ans:1 },
      { q:"What symbol means OR in a case pattern?", options:["or","||","|","&"], ans:2 },
      { q:"What Python version added match/case?", options:["Python 2.7","Python 3.8","Python 3.10","Python 3.12"], ans:2 },
      { type:"fill", q:"Fill in the blank to add a default case", code:'match x:\n    case 1:\n        print("one")\n    case ___:\n        print("other")', ans:"_" },
      { type:"output", q:"What does this code print?", code:'x = "hello"\nmatch x:\n    case "hi":\n        print("A")\n    case "hello":\n        print("B")\n    case _:\n        print("C")', ans:"B" },
    ]
  },
];
